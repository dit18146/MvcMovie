...
public class KendoController : Controller
{
    ...
    public ActionResult Upload_File(FileModel model)
    {
        ...
        fileName =
            $"{DateTime.UtcNow.ToString("yyyy_MM_dd_HH_mm_ss")}_{Guid.NewGuid()}{Path.GetExtension(model.File.FileName)}";

        newsletterId = int.Parse(Regex.Match(model.File.FileName, "\\d+").ToString());
    ...
    public int Read_File()
    {
        ...
        using (var csv = new CsvReader(reader, CultureInfo.InvariantCulture))
        {
            ...
            catch (Exception ex)
            {
                ...
                if (ex.Source == "System.Data.SQLite")
                    return 500;
            }
